...
class StreamChunk:
    ...
    Attributes:
        ...
        tool_calls: Вызовы инструментов, запрошенные моделью в этом раунде.
            Адаптер отдаёт их ровно один раз — в финальном фрагменте раунда,
            целиком собранными из дельт потока (``delta`` при этом пустой);
            в остальных фрагментах поле равно None.
    """
