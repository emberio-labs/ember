...
### Инструменты в потоковом режиме

`stream_run()` исполняет инструменты так же, как `run()`: пока модель
запрашивает вызовы, агент выполняет их, возвращает результаты обратно
модели и продолжает генерацию. Текст уходит потребителю по мере появления —
включая промежуточные реплики перед вызовом инструмента:

```python
for fragment in agent.stream_run("Какая погода в Москве?"):
    print(fragment, end="", flush=True)
```

Две особенности, о которых стоит помнить:

- Поток не разделён на раунды: реплики перед вызовами инструментов и
  финальный ответ идут подряд, различить их по одному потоку строк нельзя.
  Если это важно, используйте непотоковый `run()`.
- На время исполнения инструмента (shell-команда, вызов MCP-сервера) поток
  молчит: `Iterator[str]` не умеет сообщать «идёт работа».
...
- `skills_dirs=None` (по умолчанию) — скиллы выключены: ни инструментов,
  ...
  задаёт потребитель. Инструменты и скиллы работают и в `run()`, и в
  `stream_run()`.
